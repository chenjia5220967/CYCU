...
class AutoEncoder(nn.Module):
    def __init__(self):
        ...
        self.encoder2 = nn.Sequential(
            ...
            nn.BatchNorm2d(64,momentum=0.9),
            nn.ReLU(),
            nn.Conv2d(64, 32,kernel_size=(1,1), stride=1, padding=(0,0)),
            nn.BatchNorm2d(32,momentum=0.9),
            nn.ReLU(),
            nn.Conv2d(32, P, kernel_size=(1,1), stride=1, padding=(0,0)),
            nn.BatchNorm2d(P,momentum=0.9),
        )
    ...
    def forward(self,x):
        abu_est1 = self.encoder1(x).clamp_(0,1)
        re_result1 = self.decoder1(abu_est1)
        abu_est2 = self.encoder2(re_result1).clamp_(0,1)
        re_result2 = self.decoder2(abu_est2)
        return abu_est1, re_result1, abu_est2, re_result2

def weights_init(m):
    nn.init.kaiming_normal_(net.encoder1[0].weight.data)
    nn.init.kaiming_normal_(net.encoder1[4].weight.data)
    nn.init.kaiming_normal_(net.encoder1[7].weight.data)
...
for epoch in range(EPOCH):
    for i, (x,y) in enumerate(train_loader):
        ...
        total_loss =Loss_fuction(abu_est1, re_result1, abu_est2, re_result2)
        optimizer.zero_grad()
        total_loss.backward()
...
print('时间花销', time_end-time_start)

save_path = str(dataset) +"_"+"_"+model_2D+ str(beta)+'_7cycunet_result.mat'
